...
docker-build-quick: ## Build docker image with the manager quickly
	$(CONTAINER_TOOL) build -t ${IMG} -f ./Dockerfile.quick .
	kubectl --context kind-newyork delete multicluster newyork --wait=false &>/dev/null || true
	kubectl --context kind-sheffield delete multicluster sheffield --wait=false &>/dev/null || true
